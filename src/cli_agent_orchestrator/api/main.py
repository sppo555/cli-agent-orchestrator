...
async def run_step(
    ...
    try:
        if body.execution_mode == "structured":
            # run_structured_worker_step has no `model` parameter: the
            # structured contract picks the provider's own configured model.
            # body.model is therefore only honored on the interactive path
            # below, matching upstream's own scope for the override (#501).
