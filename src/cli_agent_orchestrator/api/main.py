...
async def run_step(
    ...
    try:
        ...
        if body.execution_mode == "structured":
            # run_structured_worker_step takes neither `model` nor the workflow
            # replay/worktree parameters: the structured contract picks the
            # provider's own configured model and creates no journalled step.
            # Those overrides are therefore honoured on the interactive path
            # only, matching upstream's own scope for them.
