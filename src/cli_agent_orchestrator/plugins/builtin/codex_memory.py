...
class CodexMemoryPlugin(CaoPlugin):
    ...
    def _write_block(self, target: Path, context_block: str) -> None:
        ...
        # AGENTS.md is user-authored and shared by every terminal in the repo.
        # The previous temp-file idiom read outside any lock and two writers'
        # finally-unlink could delete each other's live temp file, so
        # concurrent terminals could lose a block or publish a half-written
        # file. locked_atomic_rewrite holds an inter-process lock across the
        # whole read-strip-replace cycle.
