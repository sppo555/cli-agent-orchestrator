...
class AgentProfile(BaseModel):
    ...
    # grok_cli-only now. Per-agent reasoning effort, passed as `--effort <level>`
    # (e.g. "low" / "medium" / "high"). Claude Code moved to `claudeConfig`
    # upstream (#283) and codex uses `codexConfig`; this field now serves
    # grok_cli, whose provider has no config-dict equivalent.
