...
The same rule applies to documentation. As of 2026-07-26 the integration-only
customization documents no longer live at the repository root: they are
archived under `docs/` on their **owning customization branch**, so the
force-rebuild picks them up through the normal `--no-ff` merge instead of a
manual `git checkout <old-integration-tip> -- <doc>` restore.

| Document | Owning branch | Path |
| --- | --- | --- |
| Fork-sync flow, customization progress, CAO-Tailscale change summary, dispatch-UNKNOWN report | `custom/4.19-memory-scope-isolation` | `docs/fork-sync/` |
| Memory scope isolation reviews + remediation reports | `custom/4.19-memory-scope-isolation` | `docs/reviews/memory-scope-isolation/` |
| Grok token-usage reviews, worker token-usage provider inventory | `custom/4.18-grok-cli-provider` | `docs/reviews/grok-cli-provider/` |
| Grok provider plan and prompt pack | `custom/4.18-grok-cli-provider` | `docs/plans/` |

The four cross-cutting fork-sync documents sit on 4.19 because it is the
newest customization branch and is merged on every rebuild; they are not
topically owned by memory scope isolation. If 4.19 is ever retired, move
`docs/fork-sync/` to whichever branch replaces it as the always-merged tip
rather than leaving the documents on integration only.

`AGENTS.md` deliberately stays at the repository root — it is read by tooling
that expects it there, and it is not restored from an old integration tip.

When adding a new customization document, commit it to the owning
`custom/4.x-*` branch under `docs/`, never to `cao-tailscale-integration`
directly.
...
git merge --no-ff custom/4.17.6-antigravity-native-usage
git merge --no-ff custom/4.18-grok-cli-provider
git merge --no-ff custom/4.19-memory-scope-isolation
```

Merge order matters for the last three: 4.17.6 carries the native token-usage
adapter that 4.18 builds on, and 4.19 is merged last so the archived
`docs/fork-sync/` documents land on the finished integration tree.
...
> ⚠️ **Historical note (superseded 2026-07-26).** These customization docs used to live
> ONLY on the integration branch at the repository root, so the force-rebuild
> (`git branch -f cao-tailscale-integration origin/main`) DROPPED them every sync and each
> rebuild had to restore them with `git checkout <old-integration-sha> -- <doc>`. They are
> now archived under `docs/` on their owning customization branch — see
> **Documentation Ownership Rule** above — and arrive through the normal `--no-ff` merge.
> **No manual restore step is required any more.** Entries below that mention restoring
> docs from a previous integration tip describe how that cycle actually ran; they are kept
