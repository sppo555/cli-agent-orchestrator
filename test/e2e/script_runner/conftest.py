...
(``require_cao_server``, ``require_tmux``) that skip the ENTIRE e2e session
when no server/tmux is present — that gating is correct for the
provider-handoff e2e tests but wrongly skips these self-contained ones too.
Overriding the same fixture names here (pytest fixture resolution: nearest
conftest wins) with no-ops scopes the server/tmux requirement OUT of this
subpackage only, without touching the parent module.
"""
